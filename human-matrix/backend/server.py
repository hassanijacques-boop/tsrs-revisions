...
# ─── MongoDB ──────────────────────────────────────────────────
# Connection au chargement du module.
# Si MONGO_URL n'est pas configuré, le serveur démarre quand même
# (utile pour le premier déploiement Render).
MONGO_URL_DEFAULT = "mongodb://localhost:27017"
mongo_url = os.environ.get('MONGO_URL', '')
db_name = os.environ.get('DB_NAME', 'human_matrix') or 'human_matrix'
client = None
db = None
...
if mongo_url:
    try:
        ...
        logger.error(f"❌ MongoDB échec: {e}")
        logger.warning("⚠️  Le serveur démarre sans base de données. Configurez MONGO_URL dans Render.")
        client = None
        db = None
else:
    logger.warning("⚠️  MONGO_URL non défini. Le serveur démarre mais les fonctionnalités DB sont désactivées.")
